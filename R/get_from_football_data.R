...
get_from_football_data <- function(
  competition_id,
  season_ids = NA,
  path){
  ...
  if(is.na(season_ids) & data_type == "main_league"){
  ...
  if(data_type == "main_league"){
    ...
    results_urls <- paste0("https://www.football-data.co.uk/mmz4281/" ,
                           seasons_short,
                           "/",
                           metadata_football_data$url_suffix,
                           ".csv")

    results_filenames <- paste0(path, competition_id, "_",
                                season_ids, ".csv")

  } else if(data_type == "extra_league"){
    results_urls <- metadata$football_data$url_full
    results_filenames <- paste0(path, competition_id, "_all",
                                ".csv")
  } else {
